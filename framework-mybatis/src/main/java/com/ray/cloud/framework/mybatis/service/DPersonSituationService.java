package com.ray.cloud.framework.mybatis.service;

import com.ray.cloud.framework.mybatis.entity.DPersonSituation;
import com.ray.cloud.framework.mybatis.entity.DPersonSituationExample;
import org.springframework.stereotype.Service;

/**
 * @Author: ZhangRui
 * @Description:
 * @date: Created in 11:03 2018/4/2
 * @Modified By:
 */
@Service
public class DPersonSituationService extends BaseService<DPersonSituation,DPersonSituationExample> {
    public DPersonSituationService(){}
}
